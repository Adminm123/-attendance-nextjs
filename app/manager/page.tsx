...
const lbl: React.CSSProperties = {
  fontSize: 11, color: 'var(--muted)', marginBottom: 5, display: 'block',
  letterSpacing: '.06em', textTransform: 'uppercase' as const, fontWeight: 500,
};
...
export default function ManagerPage() {
  ...
  const handleTransfer = async (name: string, newBranchId: string) => {
    try {
      ...
      const data = await res.json();
      if (data.success) { toast('ย้ายสาขาสำเร็จ', 'success'); loadData(true); }
  ...
  const handleStatus = async (name: string, status: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { toast('อัพเดทสำเร็จ', 'success'); loadData(true); }
    } catch { toast('เกิดข้อผิดพลาด', 'error'); }
  };
  ...
  const handleAddBranch = async () => {
    ...
    const gpsVal = branchForm.gps.trim()
    ...
    try {
      ...
      if (data.success) {
        ...
        setBF({ id: '', name: '', province: '', totalStaff: '', minStaff: '', gps: '', radius: '50', openTime: '09:00', closeTime: '18:00' });
        setTab('branches'); loadData(true);
      } else { toast(data.error || 'เพิ่มไม่สำเร็จ', 'error'); }
    } finally { setAB(false); }
  };
  ...
  // MAIN VIEW
  // ══════════════════════════════════════════════════════════════
  const readyCount   = staffList.filter(s => s.status === 'Active' && s.hasDescriptors).length;
  const pendingCount = staffList.filter(s => s.status !== 'Inactive' && (!s.lineId || !s.hasDescriptors)).length;
      ...
      <div className="shell" style={{ paddingTop: 20 }}>
        ...
        <div style={{ paddingBottom: 20, borderBottom: '1px solid var(--line)', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{ fontSize: 30, fontWeight: 800, color: 'var(--navy-900)', lineHeight: 1.1, letterSpacing: '-.02em' }}>
                จัดการระบบ
              </div>
              <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 6 }}>
                พนักงาน {staffList.length} คน · สาขา {branches.length} แห่ง
              </div>
            </div>
            <button
              onClick={() => loadData()}
              disabled={refreshing}
              style={{ marginTop: 4, fontSize: 13, color: 'var(--navy-900)', background: 'none', border: '1px solid var(--line)', borderRadius: 9, padding: '7px 16px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, opacity: refreshing ? .5 : 1, flexShrink: 0 }}
            >
              {refreshing ? '...' : 'รีเฟรช'}
            </button>
          </div>
          ...
          <div className="kpi-grid" style={{ marginTop: 20 }}>
            {[
              ...
              { lbl: 'พร้อมใช้',    val: readyCount,        cls: 'acc-green' },
              { lbl: 'รอดำเนินการ', val: pendingCount,      cls: 'acc-warn'  },
            ].map(k => (
        ...
        {tab === 'add-branch' && (
          <div className="card" style={{ padding: '22px 20px' }}>
            ...
            {/* GPS — one combined field */}
            <div style={{ marginBottom: 8 }}>
              <label style={lbl}>พิกัด GPS (latitude, longitude)</label>
              <input
                type="text"
                placeholder="15.114112, 104.323573"
                value={branchForm.gps}
                onChange={e => setBF(p => ({ ...p, gps: e.target.value }))}
                style={inp}
              />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 8 }}>
              <div style={{ fontSize: 11, color: 'var(--muted)' }}>
                {branchForm.gps && parseGps(branchForm.gps)
                  ? `lat ${parseGps(branchForm.gps)!.lat}  lng ${parseGps(branchForm.gps)!.lng}`
                  : branchForm.gps ? <span style={{ color: 'var(--red)' }}>รูปแบบไม่ถูกต้อง</span> : null
                }
              </div>
              <div>
        ...
        <div style={{ textAlign: 'center', padding: '28px 0 12px' }}>
          <button onClick={() => setView('login')} style={{ fontSize: 12, color: 'var(--muted)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            ออกจากระบบ
          </button>
        </div>
