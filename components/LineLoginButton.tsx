'use client';
// ─── LINE Login Button ────────────────────────────────────────────────────────
// ปุ่มสำหรับ Login ด้วย LINE
// 'use client' เพราะใช้ onClick (event handler ทำงานใน browser เท่านั้น)
...
export default function LineLoginButton({ className }: Props) {
  // เมื่อกดปุ่ม → เรียก API เพื่อรับ LINE Login URL → redirect ไป LINE
  const handleLogin = async () => {
    const res  = await fetch('/api/auth/line-url');
    const data = await res.json();
    // redirect browser ไปที่ LINE Login page
  ...
  return (
    <button
      ...
      style={{ background: '#06C755', color: '#fff', borderRadius: '16px' }}
    >
